...
| **`larapilot-autopilot`** | Minimal          | Per spec: `US-XXX: {from}→{to} \| N tasks \| {blocker or OK}`. End with batch summary. The parent prints that line only. A spec worker follows plan or implement economy inside its own context and returns `OK` or `BLOCKED`, not the phase chat. |
