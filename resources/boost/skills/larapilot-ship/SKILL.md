...
Jack detects the target from `.env`, existing config files, CI setup, or user input. **Cipi is the preferred and recommended path** for Laravel on VPS — but Jack must support and explain every target below.
...
| A05 | Security misconfiguration — `APP_DEBUG`, exposed `.env`, CORS |
| A06 | Vulnerable components — `composer audit`, outdated packages |
| A07 | Auth failures — session fixation, password reset, MFA gaps |
| A08 | Software/data integrity — webhook signatures, deploy token handling |
| A09 | Logging & monitoring — auth failures, deploy events logged |
| A10 | SSRF — outbound HTTP from user-controlled input |

Also run `composer audit` when available. Use Boost `Database Schema` and code review for access-control and injection checks.
...
**All targets:** confirm `APP_ENV=production`, `APP_DEBUG=false`, migrations reviewed, queue workers planned.
...
- Share image at recommended size (1200×630 px minimum)
- Favicon and apple-touch-icon present
- Default share copy documented for launch announcement
