...
3. **Branch by Project Kind** (see branching rules in shared-runtime):
    ...
    - **Website** — round 2 AskQuestion: **Website Type** (`Showcase`, `Portal`, `Blog`, `E-commerce`, `Landing`, `Documentation`, `Other`) and **delivery target** (`MVP`, `V1 Complete`, `Full Product`). Aurora asks **Budget Sensitivity** in the same round or right after (default **Tracked** for **E-commerce**). Bring in **Emma**, **Lauren**, **Elise**, **Marika** early; **Sebastian** + **Matt** for payments/shipping on **E-commerce**; **Joe** when rich frontend or animations matter; **Albert** when **Documentation** type or technical docs are required; **Ricky** when mobile/hybrid app or device APIs are in scope; skip multi-tenancy unless **Portal** with accounts. **John + Joe** ask **Frontend Topology** (`Laravel-coupled` | `SPA-in-Laravel` | `API + external frontend`) before deep UI stack choices.
    - **Application** — full discovery: **Mark** asks **delivery target** (`MVP`, `V1 Complete`, `Full Product`, `Enterprise`); **Aurora** asks **Budget Sensitivity** in the same round or right after; **John + Joe** ask **Frontend Topology**; **John** opens multi-tenancy and admin-panel questions when signals match; full persona roster as needed.
...
**Frontend stack (in-repo):** {{Blade / Livewire / Inertia+Vue|React|Svelte / Vite SPA / N/A}}
**External frontend repo:** {{URL — when API + external frontend; else N/A}}
**External frontend stack:** {{React / Vue / Angular / Svelte / Other / N/A}}
**Companion sync:** skill + API pull | git PR | manual | N/A

### Stack

- Laravel {{VERSION}} (detect via Boost Application Info)
- Frontend topology: {{as above — John + Joe; asked via AskQuestion, never assumed}}
- Admin panel: {{Filament / Starter Kit (livewire|react|vue|svelte) / custom — asked via AskQuestion, never assumed; recommendation driven by the specific case and mockup fidelity; prefer Filament/custom when topology is API + external frontend}} — John
...
- API & docs: {{REST/OpenAPI depth per delivery target; required and versioned when topology is API + external frontend}} — John + Albert
- Companion (when external FE): `/larapilot-frontend-companion` in FE repo + `GET /larapilot/api/companion` or `larapilot:companion-export` — Joe + Matt
- ...
...
- Topology: {{Laravel-coupled / SPA-in-Laravel / API + external frontend}}
- Stack (in-repo): {{Blade / Livewire / Tailwind / Vue / React / Svelte / Flux / Filament / Starter Kit variant / N/A}}
- External FE: {{stack + repo — when external; else N/A}}; companion sync {{skill+API / git PR / manual}}
