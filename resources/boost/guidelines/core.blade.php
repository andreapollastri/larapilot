...
**Project settings:** `.larapilot/config.yaml` → `settings` (`effort`, `backlog`, `git_mode`, `testing`, `auto_approve`, `lucille`, `decision_log`, `code_history`, `release_mode`, `project_docs`, `comments`, `dashboard_auth`, `api_auth`, `security_scan`, `github`, `gitlab`, `bitbucket`, `azure`, `notifications`, `notify_slack`, `notify_discord`, `notify_telegram` — set via `/larapilot-settings`, exposed on `config-show` as `data.settings`). Every skill must read and honor `data.settings` before planning or implementing — canonical matrices in `.larapilot/shared-runtime.md` → **Project Settings**. Note: `GITFLOW` never auto-pushes (only `GITFLOW_PUSH` does); `ECO` never spawns sub-agents and **disables Lucille automatically** (re-enable with `larapilot:settings-set --lucille=YES`); boolean settings are `true`/`false` in YAML and `YES`/`NO` in envelopes; **Lucille, the decision journal (`decision_log`), and internal feedback comments (`comments`) are ON by default**; record every explicit user choice with `larapilot:decision-log` and check `larapilot:decision-check` before overriding one; **`code_history`, `release_mode`, `project_docs`, `dashboard_auth`, `api_auth`, `security_scan`, GitHub/GitLab/Bitbucket/Azure DevOps + notifications are OFF by default**; external FE repo path lives in `LARAPILOT_FRONTEND_REPO_PATH` (`.env`) — never commit user-specific absolute paths in YAML (setup in `.larapilot/integrations.md`).
