...
**Discovery interview (`larapilot-inception`):** a guided conversation (not a one-shot form). Before the interview, drop client docs in **`.larapilot/client-materials/`** and legacy snapshots in **`.larapilot/legacy/`** — skills always read them. When **`.larapilot/legacy/`** has content, Mark (with **Sabrine**) proactively asks via **AskQuestion** whether to pursue a **legacy rewrite/port** before deep discovery. Mark opens with **Project Kind** (`Personal`, `Website`, `Application`) — the first layer that switches persona depth and follow-up questions. **Personal** → lean path (MVP/V1, budget defaults `Relaxed`, business personas silent). **Website** → type next (showcase, portal, blog, e-commerce, landing, docs) then delivery target; Emma/Lauren/Elise/Marika lead; Joe when rich frontend matters. **Application** → full discovery: delivery target (`MVP` … `Enterprise`), Budget Sensitivity (`Tracked` or `Relaxed`), **Frontend Topology** (`Laravel-coupled` | `SPA-in-Laravel` | `API + external frontend` — John + Joe, before panel route), multi-tenancy, admin panel or authenticated dashboard route (Filament vs Laravel Starter Kit vs custom), integrations, compliance. When topology is **`API + external frontend`**, use **`/larapilot-frontend-companion`** in the FE repo plus `GET /larapilot/api/companion` or `larapilot:companion-export` to keep the shared PRD in sync. **Sabrine** leads legacy rewrite/port analysis, **content scraping**, and **DB/assets porting** when `.larapilot/legacy/` has content. **Andrew** advises Laravel ecosystem best practices; **Marika** owns copy; **Joe** owns frontend engineering, **design system consistency with Elise**, and visual impact. Jennifer explores market and positioning when relevant; Benjamin brings enterprise research on Application; Sebastian proposes integrations, **competitor data porting**, and runs **deepsearch** on reference product URLs (reports in `.larapilot/research/reference-products/`); John and Aurora co-own scalable, budget-aligned architecture; legacy rewrite/port preserves **all features and data** unless explicitly scoped out. **Jack asks local dev environment** (Sail, Herd, not defined yet, or other) via AskQuestion — never assumes Docker/Sail; **Jack asks deploy platform, edge/CDN/WAF, and cloud** — never assumes Cipi, Cloudflare, or AWS; recommends Cloudflare and AWS when feasible. Ask at most 3 critical questions per round (skippable); present fixed options via **AskQuestion**, not plain-text A/B/C lists. Each functional requirement in the PRD carries a **MoSCoW** tag (`Must` / `Should` / `Could` / `Won't`); `larapilot-spec` uses it to bootstrap or defer backlog specs. Write and validate the PRD before creating any backlog.

**Post-inception shortcuts:** **`larapilot-feature`** — mini-inception for one new evolutiva → spec; **`larapilot-bug`** — Sophia-led bug triage → fix spec or rework; **`larapilot-frontend-companion`** — sync shared PRD into an external frontend repo when topology is split. The PRD is a **living product contract** — updated selectively on scope changes (features, requirement gaps), not on every bug or rework; see **PRD Living Document** in `.larapilot/shared-runtime.md`.

**Vendor & package policy:** prefer Laravel built-ins/first-party (including **[Laravel Starter Kits](https://laravel.com/starter-kits)** when they fit), then **Spatie** packages (spatie.be/open-source/packages) for third-party functionality; for admin/control panels and authenticated dashboards never assume **Filament** (filamentphp.com) — explicitly ask **Frontend Topology** first, then **Filament vs Starter Kit (Livewire/React/Vue/Svelte) vs custom**, recommending the best fit for the specific case and the option closest to the project mockups. Always verify a package is maintained, compatible, and secure (`composer audit`) before requiring it.
