...
- **`/larapilot/economics` is an interactive pricing tool** — a console of dropdowns at the top of the page (hourly rate · target margin · commercial discount · team size · monthly overhead · maintenance · account type · country · tax regime · VAT mode · how it is sold · monthly list price · BASE/PRO/PREMIUM price line · market scenario · churn · growth · planning customers) recomputes the whole page, every chart, and the downloadable quote on each change. It computes **server side**, through the same `EconomicsService::snapshot()` the CLI uses, so the tax engine is never reimplemented in the browser: each change fetches `/larapilot/economics/panel` with the changed values and swaps the panel, and without JavaScript the same form submits to `/larapilot/economics`. Only values that differ from the saved profile travel in the URL, so the query stays readable and the dot next to a control means it really was changed.
- **A simulation never writes anything** — the dashboard has no write path. A what-if prints the exact `php artisan larapilot:economics-set …` command that would make it the project profile, with a copy button, and the stored cost board keeps tracking the saved profile. `GET /larapilot/api/economics?hourly_rate=70&tier=premium&discount_pct=10` does the same for an agent. Out-of-range values are dropped rather than silently clamped, so an edited URL cannot push a figure the engine would refuse.
- **Commercial discount** (`discount_pct`, `--discount`) — taken off the list price, not off the cost, so it comes straight out of the margin. The quote now carries `list_price`, `discount`, `margin_after_discount`, `margin_after_discount_pct`, and `below_cost`, and the dashboard says plainly when a discount has priced the project under what it costs to deliver.
- **Team size** (`team_size`, `--team-size`) — people working in parallel. It divides the elapsed calendar months and nothing else: person-months, overhead, and the client price are identical for one person or four, and the effort block reports `solo_months`, `person_months`, and `team_size` alongside the existing figures.
- **BASE / PRO / PREMIUM packaging** — three price lines for a subscription product, each with its own contribution per customer, break-even customers, customers needed to repay the build in twelve months, and the slice of the backlog it carries. PRO is the configured list price; BASE and PREMIUM are derived from it (0.6× / 2.2×, snapped to a shelf price) until research says otherwise. The **price line** dropdown decides which one every projection below runs on.
- **Three-line business plan** — pessimistic, realistic, and optimistic, each a full 36-month projection on the selected price line: customers at month 12/24/36, growth and churn, leads needed, the first profitable month, the month the build is repaid, and cash at month 36.
- **Market research from the team** — `.larapilot/economics.market.yaml` (`paths.economics_market`), written by Jennifer (positioning) and Benjamin (market) during `/larapilot-economics` and persisted with `php artisan larapilot:economics-market-write --file=…`. It holds the sector and segment, named competitors with their list price, price trend and year-on-year move, demand scenarios, packaging tiers, risks, and sources. The dashboard plots the competitor table with its trend, and where the selected price sits against the researched set (cheaper/pricier count, range, median, delta vs median). Larapilot normalizes and plots that research; it never invents a competitor, a price, or a trend.
- **Effort by epic and by release** — `effort.by_epic` and `effort.by_release` group the per-story hours the way the project is delivered, with stories, points, hours, and labour per group, and every breakdown row now carries its epic and release.
- **The maintenance retainer is priced on the inception answers** — delivery target, who manages the server and who is on the hook when it is down, the support window, budget sensitivity, and the ship method (`release_mode`, `git_mode`, `settings.testing`, `security_scan`) each move it by a stated amount from a 12% baseline. `/larapilot/economics` shows the whole arithmetic under **What the retainer is priced on**, what the client gets for it, and which questions inception never asked — an unanswered server question prices the retainer as application-only instead of pretending. A profile nobody configured adopts the recommendation; an explicit `--maintenance` always wins.
- **Inception asks the questions the quote depends on** — three new core rounds that fire whatever the branch and whatever the legacy answer: **Delivery Target** (existed, but a legacy or client-brief opening used to swallow it), **Business Model** (`Client project` · `SaaS subscription` · `E-commerce` · `Licensed package` · `Internal tool`), and **Operations & support** (`Server Management`, `Ops Owner`, `Support Window`). They are persisted through `larapilot:choices-set --business-model= --server-management= --ops-owner= --support-window=`, scraped from the PRD, shown on the Inception dashboard, and read by Economics: the stated business model sets the product model instead of guessing from PRD keywords, and the operations answers price the retainer. A skipped round is recorded as `Not decided`, never as a guess.
- **The client quote gained two chapters, between the capabilities and the price** — **Infrastructure and hosting**, rendered only when inception actually decided one: platform, who manages the server, who is on the hook operationally, backups, monitoring, certificates and domain, support window, and the estimated running cost, closing on the two facts a buyer needs (the running cost is billed by the provider and is not in the build price; the environment, domain, and data stay in their name). What the document promises follows who operates — daily backups with restore testing when we run the server, the platform's guarantees on a managed one, the client's IT when it is theirs — and with nothing decided the chapter is omitted rather than invented. And **Security and quality**, the chapter that sells what a demo cannot show, written for a buyer: independent review, acceptance criteria written before the build, the testing bar this project actually runs, OWASP practice, no credentials in the code, security updates inside the retainer, closing on ownership and no lock-in. Every claim is derived from real settings — the security-scan line only under `security_scan: YES`, releases only under `release_mode: YES`, GDPR only when the PRD says personal data is in scope — so the document never promises a practice the delivery does not follow. The built-in template renders both in en/it/es/fr, and `/larapilot-economics` writes them in the PRD's own language.
- **Developer domain docs — `.larapilot/docs/devs/`, always on** — every spec that changes a domain's behavior now leaves behind the reasoning that produced it: one Markdown file per domain/entity/feature (`billing.md`, `user-authentication.md`, `webhook-ingestion.md`) with a fixed skeleton — `Purpose`, `Functional flow`, `Technical design`, `Architectural choices` (including **the alternatives that were rejected and why**), `Key decisions & invariants`, `Extension points & gotchas`, `Related`. Three rules make them hold: **always English** whatever language the PRD uses, **updated in the same spec that changes the behavior** (a task is not `task-done` while its doc describes the old code), and **never deferred** — unlike README/diagram/runbook work they survive `effort: ECO`, terser but written. New runtime pack `.larapilot/runtime-dev-docs.md` carries the contract; the folder ships with its own `README.md` and `TEMPLATE.md` on install and is never overwritten afterwards. New path key `paths.dev_docs`.
- **The freshness gate** — Robert's review prompt in `/larapilot-implement` now flags any domain whose code moved in the diff while its doc did not as a **High** finding (`stale dev doc — {file}`), fixed by the parent before `spec-review`. `/larapilot-review` blocks on a touched domain with no current file — including under `ECO`, where the OpenAPI exception now has a sibling. `/larapilot-ship` blocks the release checklist on a stale or missing domain doc for a shipped feature.
- **First-change catch-up — a project with no docs is brought level, not filled in gradually** — when `config-show` reports `data.dev_docs.documented` as `false` and the codebase already has domains to describe, the first spec, fix, or hotfix inventories **every** existing domain (from `spec-list`, the codebase analysis, models, routes, service namespaces, jobs, commands, integrations), writes a file for each, commits the backfill on its own as `docs(US-XXX): bring developer domain docs level`, and only then runs its own work. No AskQuestion — the catch-up is not a decision the user arbitrates — no partial pass, and no `ECO` exemption, since "later" is what produced the empty folder. Where the original reasoning cannot be recovered from git history, the PRD, `decisions.yaml`, or the plans, the file carries `<!-- TODO: verify -->` instead of an invented motive. `/larapilot-adopt` now covers every domain the analysis found rather than a sample, so a brownfield project reaches its first spec already level.
- **`config-show` and `doctor` report the catch-up state** — new `data.dev_docs` block on the setup envelope every skill already reads first: `path`, `documented`, `count`, and the `domains` slugs present (`README.md` and `TEMPLATE.md` never count as domains). `larapilot:doctor` gained a `dev_docs_scaffold` check and the same `dev_docs` block, so the trigger is a deterministic field rather than a glob the agent has to invent.
- **Domain docs as a plan deliverable** — `task-templates.md` gained a `## Domain Docs` section on the entity, non-entity, and fix/rework task templates, and `/larapilot-plan` now names the file each task must leave current. `/larapilot-adopt` bootstraps one best-effort file per domain from the codebase analysis, marking anything inferred with `<!-- TODO: verify -->`; `/larapilot-autopilot` writes them on unattended runs; `/larapilot-bug` corrects them when a fix changes behavior.

### Changed

- **The Economics dashboard is rebuilt around the decision** — six long explained sections became a console plus five: the numbers that decide it · where the hours come from (with the by-release/by-epic totals and the per-story table) · what the client pays and what you keep · packaging · business plan · the market. The glossary moved into a collapsed block at the end instead of sitting between the figures.
- **`larapilot-economics` skill** gained a market-research step: Benjamin asks sector, segment, and how far it should spread; Jennifer researches named competitors and challenges the price against them; the result is persisted with `economics-market-write`. The rate round gained discount and team size, with the warning that a discount at or above the margin prices the project under cost.
- **Overhead follows person-months** instead of calendar months, which is what makes team size change the timeline without changing the price.
- **Discovery is a conversation, not a questionnaire** — new **Conversation & Goal Challenge** contract in `runtime-discovery.md`: AskQuestion only for fixed choices Larapilot persists, every answer gets a reaction before the next question, one thread at a time, assumptions stated out loud. Before any requirement is written, Mark, Jennifer, and Benjamin run at least two **challenge** exchanges on the goal itself — who has this problem and what they do instead, what changes if it works, how you will know in 90 days, the riskiest assumption, why now and why you, what would make you stop — and name contradictions (an MVP target with an enterprise feature list, a two-week deadline against a six-month scope) once, clearly, without blocking. The surviving answers become the vision, the personas, and a new `**Success signal:**` line in the PRD.
- **A legacy project no longer skips the core rounds** — the legacy proposal reorders them, never replaces them, and on a rewrite the server question is more urgent, not less: who keeps the old machine alive through the cutover is part of the scope. `larapilot-adopt` gained the same business-model and operations rounds, since an adopted codebase already has a server in production and the arrangement around it is usually undocumented.
- **Language Policy gained its first exception** — artifacts still follow the detected language (backlog → PRD → conversation), but files under `paths.dev_docs` are always English: they address whoever inherits the codebase, not the client. Emily does not localize them.
- **`ECO` docs deferral is narrower** — the "defer documentation theater" rule in `shared-runtime.md` and `runtime-delivery.md` now carries two carve-outs instead of one: OpenAPI when public/partner API routes change, and the developer domain docs.

### Docs

- Site / package version **v3.2.0**; `runtime-economics.md` documents the pricing tool, the discount and team-size semantics, packaging, the business plan, and the market research file format; README describes the console, the what-if API parameters, and `larapilot:economics-market-write`; new runtime pack `runtime-dev-docs.md` carries the developer domain-doc contract.
